...
class CourtDimensions: # Standardmaße für das Volleyballfeld.
    def __init__(self, scale=DEFAULT_SCALE):  # Konstruktor der Klasse, nimmt einen optionalen Maßstab entgegen.
        ...
        self.attack_line_y = self.net_y - 3 * scale  # Berechnet die y-Position der Angriffslinie (eigene Seite).
        self.defense_line_y = self.net_y + 3 * scale  # Berechnet die y-Position der (gegnerischen) Angriffslinie.
...
def get_intersection_with_net(player_pos: QPointF, ball_pos: QPointF, net_y: float) -> QPointF:   # Definiert eine Funktion zur Berechnung des Schnittpunkts einer Linie mit dem Netz.
    ...
    if (player_pos.y() < net_y and ball_pos.y() < net_y) or \
       (player_pos.y() > net_y and ball_pos.y() > net_y):  # Bedingung: Beide Punkte auf derselben Seite des Netzes.
        return None  # Gibt None zurück, wenn kein Schnittpunkt mit dem Netz möglich ist.
    
    if dy == 0: # Wenn Ball und Spieler auf gleicher Höhe liegen (horizontale Linie zum Netz).
        return QPointF(player_pos.x(), net_y)   # Gibt einen Punkt auf der Netzlinie mit der x-Koordinate des Spielers zurück.
    ...
    return QPointF(intersection_x, net_y) # Wichtig für die Spielerposition. Gibt den Schnittpunkt als QPointF zurück.
...
class DraggableEllipse(QGraphicsEllipseItem):  # Definiert eine Klasse für eine ziehbare Ellipse, die von QGraphicsEllipseItem erbt.
    def __init__(self, rect, label=""):  # Konstruktor der Klasse, nimmt ein Rechteck (QRectF) und ein optionales Label entgegen.
        ...
        if label:  # Wenn ein Label angegeben wurde.
            ...
            original_size = font.pointSize() if font.pointSize() > 0 else 12 # 12 ist die Schriftgröße. Bestimmt die ursprüngliche Schriftgröße oder verwendet 12.
    ...
    def mouseMoveEvent(self, event):    # Wird ausgelöst, wenn die Ellipse bewegt wird.
